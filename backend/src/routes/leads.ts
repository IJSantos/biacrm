  ...
  try {
    ...
    const result = await query(
      `INSERT INTO leads (name, phone, email, status, origin, user_id, custom_data, tags, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, phone, email, status, origin, req.user?.id, JSON.stringify(custom_data), JSON.stringify(tags), notes]
    );
    ...
    await query(
      ...
      [lead.id, req.user?.id, 'created', 'Lead criado']
    );
  ...
  try {
    ...
    if (status && status !== currentLead.status) {
      await query(
        ...
        [id, req.user?.id, 'status_changed', `Status alterado de ${currentLead.status} para ${status}`, currentLead.status, status]
      );
    } else {
      await query(
        'INSERT INTO lead_history (lead_id, user_id, action, description) VALUES (?, ?, ?, ?)',
        [id, req.user?.id, 'updated', 'Lead atualizado']
      );
    }
            ...
            if (leadData.name && leadData.phone) {
              const result = await query(
                `INSERT INTO leads (name, phone, email, status, origin, user_id, custom_data, tags)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [leadData.name, leadData.phone, leadData.email, leadData.status, leadData.origin, req.user?.id, JSON.stringify(leadData.custom_data), JSON.stringify(leadData.tags)]
              );
              const insertedId = result.rows[0]?.lastInsertRowid || result.rows[0]?.id;
              const leadResult = await query('SELECT * FROM leads WHERE id = ?', [insertedId]);
              ...
              await query(
                'INSERT INTO lead_history (lead_id, user_id, action, description) VALUES (?, ?, ?, ?)',
                [insertedId, req.user?.id, 'imported', 'Lead importado via CSV']
              );
            }
          }
