...
// Função para obter o status de exibição baseado no status do backend e custom_data
const getDisplayStatus = (lead: Lead): { label: string; bgColor: string; textColor: string } => {
  ...
  if (lead.status === 'fechamento') {
    ...
    if (displayStatus === 'visita_concluida') {
      return { label: 'Visita Concluída', bgColor: 'bg-teal-100', textColor: 'text-teal-800' };
    } else {
      // Por padrão, fechamento é venda_ganha
      return { label: 'Venda Ganha', bgColor: 'bg-green-100', textColor: 'text-green-800' };
    }
  }
  
  // Mapeamento dos outros status do backend
  const statusMap: Record<string, { label: string; bgColor: string; textColor: string }> = {
    novo_lead: { label: 'Sem Atendimento', bgColor: 'bg-red-100', textColor: 'text-red-800' },
    em_contato: { label: 'Em Atendimento', bgColor: 'bg-blue-100', textColor: 'text-blue-800' },
    proposta_enviada: { label: 'Visita Agendada', bgColor: 'bg-sky-100', textColor: 'text-sky-800' },
    perdido: { label: 'Perdido', bgColor: 'bg-orange-100', textColor: 'text-orange-800' },
  };
  
  return statusMap[lead.status] || { label: 'Sem Atendimento', bgColor: 'bg-red-100', textColor: 'text-red-800' };
};
