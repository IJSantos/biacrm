...
function LeadCard({ lead, isDraggingOver }: { lead: Lead; isDraggingOver?: boolean }) {
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`bg-white rounded-lg shadow-sm p-4 mb-3 cursor-grab active:cursor-grabbing hover:shadow-md transition-shadow ${
...
function StatusColumn({ 
  ...
  return (
    ...
    >
      ...
      >
        <div className="space-y-2 max-h-[calc(100vh-250px)] overflow-y-auto min-h-[100px]">
          {leads.map((lead) => (
            <LeadCard 
              key={lead.id} 
              lead={lead} 
          ...
          {leads.length === 0 && (
            <div className={`text-center text-gray-400 text-sm py-8 transition-all ${
